...
function event.dispatch(event_name, ...)
    ...
    if listeners[event_name] then
        -- Iterate over a snapshot so callbacks may safely register or remove
        -- listeners (for example when an application window is closed).
        local snapshot = {}
        for i, listener in ipairs(listeners[event_name]) do snapshot[i] = listener end
