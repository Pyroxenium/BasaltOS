...
-- Services are registered through auto-discovery, but session services have
-- real dependencies. In particular, auth must register its screens only after
-- UI exists, and every user.login listener must be installed before login can
-- happen. Remaining/third-party services are initialized alphabetically.
local SERVICE_INIT_ORDER = {
    "ui",
    "userfs",
    "libraries",
    "settings",
...
function kernel.initializeServices()
    ...
    local function initialize(name, svc)
        if not svc or initialized[name] then return end
    ...
    for _, name in ipairs(SERVICE_INIT_ORDER) do
        initialize(name, services[name])
    end

    local remaining = {}
    for name in pairs(services) do
        if not initialized[name] then remaining[#remaining + 1] = name end
    end
