...
function Container:mouse_release(button, x, y)
    ...
    -- Iterate ALL children unconditionally so elements with stuck states (e.g. "clicked")
    -- are always cleared, regardless of whether they are in the event registry.
    -- This is intentionally not using callChildrenEvent since that only reaches children
    -- that explicitly registered for mouse_release.
