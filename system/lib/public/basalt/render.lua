...
function Render:setCursor(x, y, blink, color)
    ...
    -- Immediate apply so cursor is visible even outside of a render cycle
    -- (e.g. programmatic focus with no visual change pending).
    -- render() will re-apply after every blit pass to correct setCursorPos drift.
    if blink then
        self.terminal.setTextColor(color or colors.white)
